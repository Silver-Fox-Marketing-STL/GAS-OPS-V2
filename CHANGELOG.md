...
### Added — `feature/data-sources` (Phase A; on branch, NOT yet deployed)
- **Configurable data sources — per-dealer header mapping.** New **Data Sources** sidebar screen (`ViewDataSources.html`): pick a dealer + a sample CSV, map each file header to a canonical column (dropdowns pre-filled from the saved mapping + exact-name match; VIN required), and **Save Mapping**. The normal **Import** screen then resolves those saved headers as aliases during column matching, so a feed that renames/reorders headers (e.g. "Vehicle Identification Number" → VIN) lands in the right columns automatically. Server API (Code.gs §30B): `getDataSchema_` (canonical column list from an optional `SCHEMA` tab, else the hard-coded 21 — fully backward-compatible), `getCanonicalHeaders`, `getDataSourcesBootstrap`, `getSourceMapping`/`saveSourceMapping` (per-dealer, in an optional `SOURCE_MAPPINGS` tab auto-created on first save), `getHeaderAliasMap` (global union used by the importer), `getSchemaColCount_` (groundwork for Phase B). **No sheet change is required to deploy Phase A** — it fail-safes to today's behavior until mappings are saved. (Phase B — adding genuinely new schema columns via timestamp relocation + dynamic column count — is designed but not built; it touches the live schema and is gated to a maintenance window.)
