 ...
 * @param {string|null} splitDealId    - Second Pipedrive Deal ID, required when the dealer
 *                                        has a billing_split configured (e.g. MBCC Sprinters)
 */
function pasteVinsAndRun(dealerKey, vins, dealId, runId, bypassFilters, userKey, splitDealId) {
  ...
  // Billing-split dealers bill the group (e.g. Sprinter) to a separate Pipedrive
  // deal — require it up front, mirroring the modal's gating.
  var billingSplit = getBillingSplit_(config);
  if (billingSplit && (!splitDealId || String(splitDealId).trim() === '')) {
    throw new Error(billingSplit.dealLabel + ' is required for this dealer.');
  }
  ...
  return runDealer(dealerKey, String(dealId).trim(), runId || null, bypassFilters === true, qrBasePath, config,
                   splitDealId ? String(splitDealId).trim() : null);
}
 ...
 *                                        group (e.g. Sprinter). Only used when the dealer
 *                                        has a billing_split configured in filtering_rules.
 */
function runDealer(dealerKey, dealId, runId, bypassFilters, qrBasePath, preloadedConfig, splitDealId) {
  ...
  try {
    ...
    // 17. Write run log entry. When a billing split produced group units, write
    //     TWO rows — each with its own deal ID, totals, and produced VINs — so
    //     the VIN Log Updater commits/rolls back each account independently.
    //     With zero group units the second row is skipped (a 0-VIN pending row
    //     could never be committed) and the primary row gets a note instead.
    setProgress_(runId, 'Writing run log...', 97);
    var duration            = Math.round((new Date() - startTime) / 1000);
    var runLogRowIndex      = null;
    var splitRunLogRowIndex = null;
    ...
    if (billingSplit && billingResult && groupMatched > 0) {
      runLogRowIndex = writeRunLog_(config, dealId,
        vins.length - groupMatched, matchedRows.length - groupMatched,
        billingTotals, outputDoc.getId(), duration, errors,
        billingResult.primary.producedVins, 'SPLIT:PRIMARY');
      splitRunLogRowIndex = writeRunLog_(config, splitDealId || '',
        groupMatched, groupMatched,
        groupTotals, outputDoc.getId(), duration, errors,
        billingResult.group.producedVins, 'SPLIT:' + billingSplit.groupName);
    } else {
      var runNote = (billingSplit && billingResult)
        ? 'split: 0 ' + billingSplit.groupName + ' units' : '';
      runLogRowIndex = writeRunLog_(config, dealId, vins.length, matchedRows.length,
        billingTotals, outputDoc.getId(), duration, errors, producedVins, runNote);
    }
    ...
    return {
      ...
      splitRunLogRowIndex: splitRunLogRowIndex,
      splitGroupName:      (splitRunLogRowIndex !== null) ? billingSplit.groupName : null,
