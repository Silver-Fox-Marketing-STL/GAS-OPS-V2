...
| QR Drive folder | Keeps a resolvable col-D id; otherwise find-or-create by key **under the parent derived from an existing dealer's `qr_folder_id`** (`ndQrParentFolder_`) — **never at Drive root**, so the folder inherits the shared drive's ownership instead of becoming My-Drive-owned (owner-only trash). No derivable parent = a `failed` step telling the user to paste an ID into col D. |
