...
### System Documentation | Last Updated: July 8, 2026
...
- **Writes** a dedicated **`SF_LOT_SUBMISSIONS`** spreadsheet + a **"SF Lot Submissions"** Drive folder (one subfolder of photos per dealer). Both are created by **`setupLotScannerResources()`** (run once from the editor), which logs their IDs.
- The main app's **VIN Inbox** view is the office side; the IDs are pasted into a constant in **both** projects (`LOT_SUBMISSIONS_SHEET_ID`).
...
- Append-only: an older 20-col header (pre-`batch_id`/`ocr_state`) auto-upgrades on next access (`getOrCreateLotSubmissionsSheet_`).

### Two capture paths (both off the dealer picker)
**📷 Scan a VIN (camera) — real-time, synchronous, guided.** The client decodes the label's barcode first (**ZXing**, loaded from CDN: 2D **Data-Matrix / QR / PDF417** then 1D **Code 39 / 128** — CDK windshield labels encode the VIN in a 2D Data Matrix, which decodes exactly), validating each decode as a real 17-char VIN; on a hit it skips OCR. Then **`submitVinPhoto(dealerKey, base64, correctedVin, submissionId, batchId)`** saves the photo to Drive, runs native Drive OCR, parses + validates the VIN, matches it against the dealer's inventory, and writes a `status=submitted` row. The result panel shows one outcome — ✅ *in inventory* (Take another) · ⚠️ *valid VIN not in this dealer's stock* (verify against the photo → **Add & next** / **Retake**) · ❌ *no VIN read* (**Retake**; on the **2nd consecutive fail** also **Submit photo only** for the office to transcribe). Retake = **`discardSubmission`** (delete the row + trash the photo). The captured photo shows inline instantly from the local image (tap to enlarge). The client downscales to ~3000px / JPEG q0.85 before base64.

**⬆ Upload batch (gallery) — asynchronous.** Decode runs **serially** (one image in memory at a time — mobile Safari OOMs decoding several ~12MP photos at once); uploads run **5 in parallel** (`uploadPhotoOnly` — Drive only, no sheet write, no OCR); the uploaded ids are buffered and written by a **single serial, chunked committer** (`commitQueuedBatch`, 10 rows/call under a brief lock — one execution writes the rows, so no concurrent appends lose any) as `status=draft, ocr_state=queued`. Then **`ensureOcrTrigger`** installs a **per-user, self-deleting time trigger** — `drainOcrQueue`, every 1 min, **runs as that user so OCR uses their own quota** — which OCRs the queued rows in the background (~8/run, with a bounded rate-limit backoff), and when the user's queue drains **emails them a `MailApp` summary** and **deletes its own trigger**. The crew can close the app after the (fast) upload; OCR finishes on its own.

### OCR engine — native Google Drive OCR
`extractVinFromImage_(blob)` inserts the photo via the **Advanced Drive Service v2** (`Drive.Files.insert({title, mimeType: blob.getContentType()}, blob, {ocr:true, ocrLanguage:'en'})`), reads the resulting Google Doc's text via `DocumentApp`, and **always trashes the temp Doc**. Free, no API key, no third-party egress; never throws (degrades to manual entry). VIN parsing (`extractVinCandidates_` + the ISO-3779 `isValidVin_` / `vinCheckDigit_`) forces the VIN-illegal O/I/Q to 0/1/0, prefers exact-valid 17-char tokens, and uses the check digit to gate the noisier last-resort confusion repairs.

### Drafts (field) + VIN Inbox (office) — "Both" finish flagged photos
- **Scanner Drafts tab** (`getMyDrafts`) — the submitter's own un-sent (`status=draft`) rows, grouped by `batch_id`, each batch a `<details>` group **collapsed by default** (header = dealer · timestamp · count · short batch id). Per-row correct-VIN / **📷 Show/hide photo** (`getPhotoDataUrl` fetches the bytes server-side → a data URL — reliable in the web-app sandbox, where a `drive.google.com/thumbnail` URL won't load — shown inline, tap to enlarge) / discard; per-batch **Send to office** (`sendDraftBatch` flips the batch's drafts → `submitted`) and **Delete batch** (`discardBatch` — trashes every photo + removes every row).
- **Main-app VIN Inbox** (`ViewVinInbox.html` + Code.gs **Section 32**) — the office processor reviews **open work** (`getVinSubmissions` with no filter = `draft` + `submitted`) grouped by dealer: a **"draft — not yet sent"** badge, a **"still processing…"** state for `ocr_state=queued` rows (actions appear once OCR finishes), the photo thumbnail, the read VIN (editable → re-validate + re-match via `updateVinSubmissionStatus`, status allow-listed `draft|submitted|processed|discarded`), valid/matched badges, **Processed / Discard**, and a per-dealer **Copy valid VINs** to paste into Run Order.

### Setup / rollout (one-time, before live use)
1. Run **`setupLotScannerResources()`** once from the scanner's editor (creates the sheet + folder, logs their IDs).
2. Paste `LOT_SUBMISSIONS_SHEET_ID` into the scanner's constant **and** the main app's inbox constant; paste `LOT_PHOTOS_FOLDER_ID` into the scanner.
3. Enable the **Drive API** advanced service (`Drive` is undefined if it's off — OCR guards on `typeof Drive`).
4. Deploy the scanner as a web app; grant the lot crew access (read config/master, edit the submissions sheet + photos folder). Adding the Mail + Trigger scopes re-prompts each user for authorization on next open.
...
| `getDealerVinData` | `(dealerKey)` | Returns a `{ "<VIN-UPPER>": {year, make, model, type, stock, status} }` map scoped to the selected dealer's SCRAPERDATA rows. Fetched once on dealer select by the Run Order live inventory table; **fail-safe `{}`** on any error. Built by the pure helper `buildVinDataMap_(rows)`. |
...
### Theme System *(`feature/theme-presets`, June 27–28, 2026 — 10 themes; committed `a27424a`, not yet deployed)*

The App is themeable from a **single curated pick** — one `data-theme` slug on `<html>`, persisted per Google account by `saveThemePreference` (validated by `isThemeSlug_`, fail-safe to Light on an unknown id). Themes are **pure presentation** (CSS `var(--…)` token overrides + optional `data-theme`-scoped layout rules) — they never touch view JS or any `google.script.run` call, so a theme **cannot break a backend link**. The registry, the axis engine, and the token blocks live in **`SharedUtils.html`**; the shell-structure CSS lives in **`App.html`**.

**Structural axes (the reusable layer).** On top of the palette, a theme composes optional **structural axes** declared in its `Theme.themes` metadata: **`shell`**, **`density`**, **`shape`**, **`nav`**, **`arrange`** (`Theme._axes`). `Theme.apply()` and **`Theme.reflectAxes(id)`** write each declared axis onto `<html>` as a `data-*` attribute (`data-shell` / `data-density` / `data-shape` / `data-nav` / `data-arrange`); **an axis the theme omits is removed → that axis's base default.** So one curated theme can reshape layout *and* elements with **zero per-combination CSS** — any palette composes any structure by just naming the axis.

- **`data-shell`** *(App.html)* — shell layout: `top-rail` (nav as a top bar) / `right-rail` (nav on the right). Default = left sidebar.
- **`data-density`** *(SharedUtils token block)* — `compact` / `spacious` spacing + type scale. Default = comfortable.
- **`data-shape`** *(SharedUtils token block)* — `sharp` (0-radius) / `flat` (no drop shadows). Default = rounded.
- **`data-nav`** *(App.html)* — `icons` (collapse the rail to an icon strip that expands on hover/focus). Default = labeled.
- **`data-arrange`** *(NEW; drives the grid-area skeletons below)* — page-layout axis, e.g. `desktop` (Luna). Default = each view's own default `grid-template-areas`.
...
| ViewHome | `.home-scroll` | `"status" "wflabel" "cards" "dashhead" "dash"` |
...
**Re-keyed existing structure (behavior-equivalent).** The original **Top-Rail** structural CSS — formerly keyed on `data-theme` (`[data-theme^="top-rail"]`) — was re-keyed onto the axes: Top Rail's shell rules → `[data-shell="top-rail"]` / `[data-shell="right-rail"]`. Same rules, now reusable by any palette. (Dark — Dense, which formerly demonstrated the `density`/`right-rail` axes, was cut from the set — see below — but the axes it proved remain.)
...
**The 10 themes.** Light, Dark, Top Rail, Top Rail — Dark, Midnight HC, Encarta, Sage, **Gruvbox Rail** (Gruvbox palette + `shell:'top-rail'`), **Slate — Icon Rail** (Slate palette + `nav:'icons'`), and **Windows XP** (`luna`). The Gruvbox-Rail and Slate-Icon-Rail presets prove palette × axis reuse with **no new CSS** (the gruvbox/slate palette token blocks are reused only by these composed variants — their standalone `data-theme` selectors were dropped). Dark palettes are added to the `.app-brand .logo` lightening selector list in `App.html`. **Cut from the experimental set:** Dark — Dense, Cyberpunk, Solarized Light, Sepia, Slate (standalone), Solarized Dark, Nord, Gruvbox (standalone), Rosé — and with Cyberpunk its `App.html` shell block + the **Rajdhani** Google Font were removed. **Adding a theme** is still: push `{id, label, …axes}` to `Theme.themes` and (for a new palette) add its `:root[data-theme="<id>"]` token block; a pure palette × axis recombination needs no CSS at all.
...
#### UI Settings — per-user nav layout, layered on top of the theme *(June 28, 2026)*

A theme picks the *default* shell, but the user can override the nav layout for their own account from the new **UI Settings** view (`ViewUiSettings.html`, under **System Settings**). It is **separate from the theme** — display preferences, not a palette:

- **Nav-layout chooser** — **Left Sidebar**, **Top Rail**, **Bottom Rail** *(new)*, **Windows Start Menu** *(new — a bottom taskbar `#startBar` with a Start button that toggles the nav as a pop-up "Start" panel via `toggleStartMenu()` (`data-start-open` on `<html>`), plus a live taskbar clock `#startClock`)*, plus an **autohide** toggle (collapse the left sidebar to an icon rail).
- **Persistence (Code.gs).** `getUiPrefs()` → `{navLayout, autohide}` and `saveUiPref(key, value)` write `ui_nav_layout` / `ui_autohide` to **UserProperties** (per Google account). Fail-safe like `saveThemePreference`: the nav layout is validated against `UI_NAV_LAYOUTS_ = ['auto','sidebar','top-rail','bottom-rail','start-menu']` — an off-list value falls back to `'auto'`, and `saveUiPref` ignores anything off-shape.
- **Application (SharedUtils).** The prefs are injected **pre-paint** as `window.UI_PREFS`; **`UiPrefs.applyOverrides()`** (run from `reflectAxes`) maps the user's nav layout onto `data-shell` and the autohide flag onto `data-nav="icons"`. A user's explicit pick **overrides** the theme's `data-shell`; **`'auto'` follows the theme** (leaves the theme's layout untouched). `UiPrefs.setAutohide(on)` persists the toggle live.
...
### Unified Component Layer *(SuperDesign "variant 1"; branch `ui-unification`, July 2026 — not yet merged to `main`)*
