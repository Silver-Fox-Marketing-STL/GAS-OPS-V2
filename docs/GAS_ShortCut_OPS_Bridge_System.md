...
### System Documentation | Last Updated: September 23, 2026
...
|---|---|---|
| September 23, 2026 | 1.0+ | **Screenshot-audit UI fixes (branch `fix/ui-audit-quick-wins`; `fb7d780`, `d863182`, `7a88a4e`, `b45711e`, `8c812b9`, `3658f3a`).** Highest-impact finding: Run Order's Inventory match table ✕ column (per-row remove + the "Remove Duplicates" control) slid off-screen once Features/editable columns pushed the table past 1440/1920 width, with no scroll hint. `th#rvXHead`/`td.rv-x-cell` are now `position: sticky; right: 0` (th `z-index: 2`) so the table scrolls UNDER the pinned column; pinned cells keep an opaque `var(--bg)` base and re-apply their row tint as a `background-image` layer (dark themes' `.row-dup`/`.row-warn`/hover tints are translucent rgba and would otherwise bleed scrolled content through); `rvUpdateScrollCue()` toggles `.rv-more-right` (left shadow) on scroll/resize/render. Follow-up: the "Remove Duplicates" button moved out of the ✕ header cell (it had capped/auto-widened that column — fine unpinned, but an opaque strip hiding a data column once pinned) into the "Inventory match" label row, right-aligned above the ✕ column. New shared `--th-bg`/`--th-fg` tokens give `.table-u th` a surface-tint header in the dark-family themes instead of the fixed inverted near-white bar (light themes unchanged; Import's snapshot-table header re-declare now reads the tokens too). New shared `.file-pick` recipe (SharedUtils) themes the native `<input type=file>` chrome — used by Import and Data Sources. Also: VIN Logs runs-table column rebalance + SPLIT badge on its own line; Home tile auto-fit; CAO caret size fix; Encarta flat-secondary bevel fix; form controls take the body font in Dealer Rules; Run Order flow-zone empty-state placeholder (CSS-only `:has()`). Harness green (134/134); verified by pixels across light/dark/gruvbox/encarta/luna at 1440 and 1920. See **Unified Component Layer** and the Run Order match-table notes below. |
| August 12, 2026 | 1.0+ | **Add Dealer wizard + FEATURES double-render fix (branch `feat/add-dealer-wizard`; merged `56b61b2`).** (1) **Add Dealer wizard** (System Settings → Add Dealer, `ViewAddDealer.html` + Code.gs **SECTION 35**) — a 3-panel flow (Basics → Review & Create → Setup Checklist) that provisions a dealer end-to-end. `getAddDealerBootstrap` returns existing keys, inactive dealers (resume strip), live scraper locations, the unconfigured-location diff, the next ORDERS column preview, and the derived QR parent in one round trip. **`createDealer` is an idempotent ensure inside ONE script lock** — ORDERS column at **max+1** (`ndNextOrdersCol_`, dealer-name header, `@` format), DEALERS row **`active=FALSE`** with the baseline `filtering_rules`, SF_VIN_LOGS tab (`ORDER_ID \| VIN \| committed_at \| order_date`; an **orphan tab is reused with its history**), and a QR folder find-or-created under the parent derived from an existing dealer's `qr_folder_id` (**never Drive root**) — so it doubles as the checklist's "Create Missing Pieces" repair action; per-artifact failures are recorded in `steps[]`, not thrown. **`getDealerChecklist`** is a sheet-reads-only completeness probe mirroring the run's real gates (blocking: `dealers_row`, `orders_col`, `vin_log`, exact-match `scraper_location` probe, `filtering_rules`, `pipedrive_org` PRIMARY row, `product_map` via `validateProductMapForRun_` over the allowed types; `qr_folder` **warn-only** per `runNeedsQR_`); **`activateDealer`** re-runs it **server-side** before flipping DEALERS col L. Pipedrive items hand off to Dealer Rules via new `rulesOpenDealer(key,name,tab)` (client-side injection of the inactive dealer into the active-only dropdown + `CustomSelect.refresh`). (2) **`importScraperData` now returns `unconfiguredLocations`** — a fail-soft diff of the review's live locations against DEALERS col J across **ALL** rows — rendered as an import-review warning banner with prefilled "Add dealer →" jumps. (3) **Fixed: `FEATURES:<header>:edit` no longer double-renders** — see **CSV Schemas → Editable columns**. Harness 125/125. See **Adding a New Dealer**. |
| July 22, 2026 | 1.0+ | **Run Order WILL-BE-FILTERED flag (branch `feat/run-filter-flag`).** The Inventory match table now flags found rows the run's filtering rules would remove (amber `row-warn` tone, "· ⚠ WILL BE FILTERED (reason)" in the status cell, "N will be filtered" in the match line) — previously such a VIN looked like a normal found row and silently vanished at runDealer step 8.5. Checking **Bypass filtering rules** clears the flags live (re-render on checkbox change). Server: `getDealerVinData` gains a `filtered` `{VIN→reason}` map via new pure `buildFilteredVinMap_` — the SAME `applyFilteringRules_` engine + `'run'` phase the run uses, so the flag cannot drift from run behavior; fail-safe `{}`. Harness 92/92. |
| July 21, 2026 | 1.0+ | **Order Drafts + Workflows-card removal (branch `feat/order-drafts`; merged `92df6b4`; deployed).** Per-user persistent drafts of in-progress Run Order work (typed VINs, per-row FEATURES text, editable-column edits, bypass checkbox) — autosaved on dealer switch, nav-away (new `VIEW_LEFT` registry), 15 s debounce, or a **Save draft** button; one draft per (user, dealer), last-write-wins in a new **`DRAFTS`** tab of SF_SYSTEM_MASTER (epochMs, never Dates). A Drafts band restores work (`runPrefillFromInbox` idiom); a successful run deletes the dealer's draft; a Home HUD "📝 N drafts" chip piggybacks `getHomeHud.draftCount`. Server `saveRunDraft`/`getMyRunDrafts`/`deleteRunDraft` all fail quiet; harness 90/90. Closes the pre-run typed-work-wiped-on-switch hole `discardPendingGuard` never covered. **Also removed** the Home Workflows launcher cards (redundant with sidebar/Start nav; grid now `status → hud`; the `arrange` theme axis stays but is now inert). See **Order Drafts** + **Home HUD**. |
| July 21, 2026 | 1.0+ | **VIN Inbox live VIN checking + card de-clutter (branch `feat/vininbox-live-check`; merged `b0ffebd`).** The card's validity/inventory tags, vehicle line, warning border, and batch "N valid" chip update as you type (one `getDealerVinData` preload per dealer + a client twin of `isValidVin_`); blur/Enter persists via `updateVinSubmissionStatus` with no full reload; the **Re-check** button, header VIN, and per-card email·timestamp are gone. |
| July 21, 2026 | 1.0+ | **Run Order dupe guard (branch `feat/run-dupe-guard`; merged `7427cf4`).** The Inventory match table flags rows whose VIN/stock already appears in the dealer's VIN log (`row-dup` danger tone, "· ALREADY PRINTED", count in the match line), with a per-row ✕ remove and a "Remove Duplicates (N)" header button. Server `getLoggedIdentifiers(dealerKey)` reads VIN-log col B, fails safe to empty; the VIN-logs-never-written-automatically invariant is untouched. |
| July 21, 2026 | 1.0+ | **Targeting operators `starts_with` / `not_starts_with` (branch `feat/starts-with-filter`; merged `b84346c`).** Case-insensitive prefix match in `conditionMatches_` + the Rules editor dropdown, same empty-value fail-safe as `contains`. First use: Weber Chevy drops stock numbers starting with "W". Harness 88/88. See **Filtering Rules → Targeting rules**. |
| July 21, 2026 | 1.0+ | **Today's Print Schedule band on the Home HUD (branch `feat/print-schedule`; merged `9db91b7`).** Each dealer's PRIMARY Pipedrive org carries a "Print Schedule" multi-select field; `getPrintSchedule` (Section 31) renders the dealers scheduled today and not-yet-run, with the System-Stats "Today" group becoming per-dealer printed cards. Field key auto-discovered then persisted; one batched v2 org list, org→days cached 6 h; RUN_LOG-derived fallback survives a Pipedrive outage. See **Home HUD → Today's Print Schedule band**. |
| July 20, 2026 | 1.0+ | **Billing CSV attach + per-schema CSV folder export (branch `feat/billing-csv-export`; merged `24bfdd5`).** The Pipedrive deal's billing artifact switched from the formatted billing **PDF** to a **CSV of the live BILLING tab** (`attachBillingCsvToDeal_`); runDealer step 14b auto-exports every CSV* tab as a real `.csv` into the output folder. The ~370-line billing-PDF machinery (`buildBillingPdfTab_` et al.) was **deleted** — any remaining billing-PDF prose is superseded. See **Pipedrive → Billing CSV attach**. |
| July 16, 2026 | 1.0+ | **FEATURES column + VersaWorks schema/header overrides + editable columns + Run Order layout rework (branch `feat/features-column`; merged `cca8232`).** `FEATURES` field code → ORDERMATCH col W (first user-typed CSV column, required per-row when the schema demands it); CSV_SCHEMAS cells gain `CODE:HEADER[:edit[N]]` syntax (`parseSchemaCell_`) so a header can print a VDP field name and a column can be made user-editable pre-run (`computeEditSeed_`/`csvCellValue_`); QR generation is now schema-gated (`runNeedsQR_`). Run Order relaid out (rail retired → top-center flow zone; deal-id fields moved to the finalize cards). New field codes `MODELTRIM` (X/24), `MV_PRICE` (Y/25) via the FIELD_CODES overlay. Harness 84. See **CSV Schemas**, **Field Codes**, **ORDERMATCH**. |
...
**Note on column indices (CFG object):** The CFG constant in Code.gs uses 0-based column indices. Key values: `KEY:0, NAME:1, ORDERS_COL:2, QR_FOLDER_ID:3, OUTPUT_FOLDER:4, LINKBUILDER_COL:6, UTM_BASE_URL:7, TRANSFORMS:8, SCRAPER_LOCATION:9, QR_PREFIX:10, ACTIVE:11, NOTES:12, PIPEDRIVE_PREFIX:13, TYPE_RULES:14, FILTER_RULES:22`. **There is no `USE_STOCK` key** — index 5 (col F `use_stock_not_vin`) is skipped entirely; the column is dormant and append-only.
...
- **condition `op`** — `in`, `not_in`, `contains`, `not_contains`, `starts_with`, `not_starts_with` (string, case-insensitive; the positive string ops match **any** value — OR), or `gte`, `lte`, `gt`, `lt` (numeric; price-safe — strips `$`/`,` before compare, since prices are stored as text). *(`gt`/`lt` were added for this model; `starts_with`/`not_starts_with` added July 2026 for Weber Chevy's "drop stock numbers starting with W"; the `drop_on_import` operator was removed — it's now an action.)*
...
**`FEATURES` is excluded from the `edit` mechanism (August 12, 2026, `f518f14`/`c724227`).** It already owns the dedicated always-editable Features input (`featuresTypesForDealer_`, which matches on the CODE and ignores the flag), so a `FEATURES:<header>:edit` cell qualified for **both** column systems and rendered **two** features fields per row (seen live on Mini of St. Louis and Spirit Lexus) — the second one half-wired: it bypassed the `collectMissingFeatures_` run gate yet still overrode the CSV value via `csvCellValue_`. `editableCodesForDealer_` now skips code `FEATURES` outright (exact match, at the single chokepoint — all callers fixed at once; other `edit` codes unchanged), and the CSV Schemas editor refuses to SAVE the flag on FEATURES (`buildCsvSchemaCells_` throws), so config and engine can't disagree. Existing flagged cells stay inert until cleaned up.
...
| `conditionMatches_` | `(row, cond)` | Evaluates one targeting condition leaf (`{field, op, values}`) against a SCRAPERDATA row via `getFilterFieldIndex_()`. Returns **bool**. Ops `in/not_in/contains/not_contains/starts_with/not_starts_with/gte/lte/gt/lt`, price-safe numeric coercion. **Fails SAFE** (no match) on unknown field/op, empty values, or unparseable number — so an exclusion can't fire on a typo. |
...
Home's raw DASHBOARD-grid dump — the generic `renderDashboardRows_`/`.dash-*` renderer and the on-screen Run Log Summary / Most Recent Run / Runs By Dealer / raw Inventory Snapshot tables — is replaced by a HUD. The status strip is unchanged; below it (the Workflows launcher cards were **removed July 21, 2026** — see the note at the end of this section): an **All-Time** strip, **Today**/**This Week** stat-tile rows (runs, VINs, dealers, dupes), and a **Dealer Focus** card. Server `getDashboardView` is **retired** (zero callers) — the **DASHBOARD sheet and `refreshDashboard_` are untouched** and still human-viewable directly in Sheets; `getInventorySnapshot` (below) now slices its inventory block for the Import view instead.
...
`ViewHome.html`: the grid-area skeleton drops `dashhead`/`dash` for a single **`hud`** area (originally `grid-template-areas: "status" "wflabel" "cards" "hud"`; **now `"status" "hud"`** after the Workflows cards were removed July 21, 2026 — see the theme grid-skeleton table below). Stat tiles (`.home-tile`: a large numeral over a small uppercase label; `.zero` mutes a zero count) render `today`/`week`/`allTime`. The **Dealer Focus** card (`select#homeDealerSelect`, populated from the `AppData` dealer list) calls `getDealerSummary` on pick and renders inventory totals + as-of, run stats + per-type breakdown, the most recent run with its VIN-log status tag, and the EOM open-order count — each section independently handles its own loading/empty/null state. A **stale-response sequence guard** (`homeSummarySeq`) discards a `getDealerSummary` response that lands after the user has already switched to a different dealer. `loadHomeHud()` re-runs on every Home visit (`VIEW_SHOWN`) and via the Refresh button, reloading the currently-picked dealer's summary alongside the HUD stats.

**Workflows launcher cards removed (`635f3e4`, July 21, 2026).** The five Home launcher cards (Run Order / Import / VIN Logs / Dealer Rules / Normalization) and their `Workflows` section label were deleted — redundant with the sidebar/Start nav and eating vertical space above the HUD. Gone with them: the `.home-card`/`.home-cards`/`.home-section-label` CSS, the `wflabel`/`cards` grid areas (the Home grid is now just `"status" "hud"`), the Encarta `.home-card` group-box override, and the entire **`[data-arrange="desktop"] #view-home .home-card` icon-grid block** in `App.html` (which turned the cards into captioned desktop icons under Luna). The **`arrange` theme axis itself stays** on the registry (Luna still declares `arrange:'desktop'`) but is now a **no-op** — no view responds to it anymore.
...
### Order Drafts — persistent Run Order work *(July 21, 2026, branch `feat/order-drafts`, `ed8694a`; merged to `main` `92df6b4`; deployed to prod July 21, 2026)*

Before this, `discardPendingGuard()` only protected the *post-run* un-finalized cards; **pre-run typed work was silently wiped on a dealer switch** (the change handler clears `vinInput`/`rvFeatures`/`rvEdits`). Order Drafts closes that hole: in-progress Run Order state is saved per user so an accidental dealer switch, nav-away, or page reload can't lose typing.

**What a draft holds** (`rvBuildDraftPayload`, ViewRun.html): `{dealerKey, dealerName, vinText, features, edits, bypassFilters, savedAt}` — the typed VIN textarea, the per-row `FEATURES` text (`rvFeatures`, keyed by VIN), the editable-column edits (`rvEdits`, VIN→code→text), and the Bypass-filters checkbox. **One draft per `(user_email, dealer_key)`, last-write-wins.**

**Storage — `DRAFTS` tab in SF_SYSTEM_MASTER** (`user_email | dealer_key | payload_json | updated_at`). `updated_at` is **epochMs (a number, never a Date** — a Date can't cross `google.script.run`). Find-or-create via `getOrCreateDraftsSheet_` (writes the header row on first use); accessed **only** through `getMasterSS_` (the one-accessor rule). The payload is one JSON cell guarded at **45 000 chars** (`RUN_DRAFT_MAX_JSON`) against the 50 k Sheets cell cap → `{ok:false, reason:'too_large'}` (the client toast tells the user to trim the features text).

**Server (Code.gs, Section 28, right after `saveUiPref`)** — three client-callable fns (no `_` suffix) + two pure helpers:
- **`saveRunDraft(payloadJson)`** — upserts this user's draft for the payload's dealer. **ScriptLock (10 s wait) around the read-scan-write** so racing autosaves can't append a duplicate row; `SpreadsheetApp.flush()` inside the lock. Returns `{ok, reason?, savedAt?}`.
- **`getMyRunDrafts()`** — this user's drafts, newest-first; each entry carries the band summary **plus the full parsed `payload`** so Resume needs no second call. Unparseable rows are skipped, never fatal. → `{ok:true, drafts:[...]}`.
- **`deleteRunDraft(dealerKey)`** — deletes this user's draft(s) for a dealer, bottom-up (tolerates legacy dupes), under the same ScriptLock.
- **`runDraftEmail_()`** — active user's email, lowercased, `''` when unavailable (no email → no draft, silently).
- **`summarizeRunDraft_(payloadJson)`** — **pure, harness-tested**: `{dealerKey, dealerName, vinCount, featCount}`; `vinCount` mirrors the run's own trim/dedupe-upper, `featCount` = non-blank features; bad JSON → zeros.

**Everything fails quiet** (the stats-side-write philosophy: a draft hiccup can never break a run, a dealer switch, or the Home HUD). No email, no dealerKey, or any exception → a benign `{ok:false}`/empty result.

**Save triggers (ViewRun.html):**
- **Dealer switch** — the change handler snapshots BEFORE it wipes state (`if (rvVinData.dealerKey !== key && rvDirty && rvHasWork()) rvSaveDraft()`), saving what the user is *leaving*.
- **Nav-away** — the new **`VIEW_LEFT` registry** (`SharedUtils.html`, declared beside `VIEW_INITS`/`VIEW_SHOWN`): `App.html` `navTo` fires `VIEW_LEFT[leavingViewId]()` before the show/hide loop. `VIEW_LEFT['view-run']` autosaves when `rvDirty && rvHasWork()`.
- **Debounced autosave** — 15 s (`RV_AUTOSAVE_MS`) after the last edit while dirty (`rvMarkDirty` on the VIN input, features/edits, and the bypass checkbox).
- **Manual** — the **Save draft** button (`rvSaveDraft({manual:true})`, toasts the result).
- A **successful run of any method (including `test`)** deletes that dealer's draft (`deleteRunDraft(lastDealerKey)` in the run success handler).

**Client single-flight** (`rvSaveInFlight`): a save that lands while another is in flight retries in 2 s **carrying its pre-wipe payload snapshot** (`opts.payload`) — so the dealer-switch path saves what the user left even though the live state was already wiped by the time the retry runs.

**Restore — the Drafts band** (`#rvDraftsBand`, `.rv-drafts-band`, above the Run body): one item per draft (dealer name, VIN/features count tags, age) with **Resume** and delete. Buttons pass an **integer index** into `rvDraftList` (never an interpolated name — apostrophe trap); dealer names are set via **`textContent`**, never HTML-interpolated. `rvResumeDraft(i)` mirrors the `runPrefillFromInbox` idiom: select the dealer through the **normal change flow** (guard + wipe + data load), then re-seed `vinInput`/`rvFeatures`/`rvEdits`/`bypassFilters` on top and `rvRenderTable()` (programmatic `.value` sets fire no events). If the dealer-switch guard is cancelled or the dealer is no longer active, the resume aborts.

**Home HUD chip** — an "📝 N drafts" chip (`#homeDraftsChip`) piggybacks on `getHomeHud`'s **`draftCount`** field (`getMyRunDrafts().drafts.length`, wrapped in try/catch — non-fatal; a draft failure never breaks Home). Clicking it jumps to Run Order.

> **`[hidden]` vs `display` (fixed same day, `0aebf5b`):** both `.tag` (`display:inline-block`) and `.rv-drafts-band` (`display:flex`) set their own `display`, which as author rules beat the UA `[hidden]{display:none}` rule — so the "0 DRAFTS" chip and the empty Drafts band painted despite `hidden=true`. Fix: `#homeDraftsChip[hidden]` and `.rv-drafts-band[hidden]` re-pin `display:none !important` (the same trap the `.view[hidden]` guard exists for — see LEARNINGS). Any display-setting component class toggled by the `hidden` attribute owes itself this override.

**Harness:** `summarizeRunDraft_` is pinned in `test/run-tests.js` (90/90 assertions).
...
- **`data-arrange`** *(drives the grid-area skeletons below)* — page-layout axis; only Luna declares it (`desktop`), but as of July 21, 2026 **no view responds to it** (its sole consumer, Luna's `#view-home` desktop-icon grid, went with the removed Workflows cards) — the axis is retained on the registry but inert. Default = each view's own default `grid-template-areas`.
...
| ViewHome | `.home-scroll` | `"status" "hud"` *(was `"…" "dashhead" "dash"` before the July 10, 2026 HUD rework, then `"status" "wflabel" "cards" "hud"` until the Workflows cards were removed July 21, 2026 — see **Home HUD** above)* |
...
**July 10, 2026 (`e9d6cf5`) — themes went strictly palette-only.** **"Top Rail" and "Top Rail — Dark" were deleted outright** — a saved pref of `top-rail`/`top-rail-dark` remaps to `light`/`dark` at read time in `getThemePreference` (no write-back; the stored slug stays as-is, but every read since resolves it to a live theme). **"Gruvbox Rail" → label "Gruvbox"** and **"Slate — Icon Rail" → label "Slate"** — both demoted to pure recolors (their `shell`/`nav` metadata dropped from `Theme.themes`); the **ids stayed** `gruvbox-rail` / `slate-icons` because ids are persistence keys — renaming them would orphan every account's saved pref. Current registry (8 entries): Light, Dark, Midnight HC, Encarta, Sage, Gruvbox, Slate, Windows XP — only Windows XP (`luna`) declares an axis (`arrange:'desktop'`), and since the Workflows-card removal July 21, 2026 that axis is a **no-op** (no view rearranges on it anymore — Luna's Home desktop-icon block was the only consumer).

Dark palettes set `--logo-fill: #d2d2d6` in their own token block (the `.app-brand .logo` mask reads `var(--logo-fill)`, default `#4a4a4d` on `:root`; the old per-theme selector allowlist in `App.html` is gone — Sep 25, 2026). **Cut from the experimental set (June 27–28):** Dark — Dense, Cyberpunk, Solarized Light, Sepia, Slate (standalone), Solarized Dark, Nord, Gruvbox (standalone), Rosé — and with Cyberpunk its `App.html` shell block + the **Rajdhani** Google Font were removed. **Adding a theme** is now: push `{id, label}` (+ `arrange` if it needs page rearrangement — `shell`/`density`/`shape`/`nav` are no longer valid metadata a theme should set) to `Theme.themes` and, for a new palette, add its `:root[data-theme="<id>"]` token block.

**Windows XP (`luna`) — the showcase of the approach** *(metadata `{ arrange: 'desktop' }`, no shell axis)*: a palette/token block in `SharedUtils` (Luna blue `#316ac5` accent, beige `#ece9d8` controls, white client, Tahoma/Trebuchet system fonts — no CDN webfont, 3px radius, `--shell-header-h:30px`), a window-frame chrome block in `App.html` (blue-gradient title-bar header, rounded-top white window over a blue desktop `#appRoot`, Luna side panel, **green** taskbar footer, glossy grey buttons, sunken white inputs; sets `--logo-fill` for the light logo), and — since the Workflows cards were removed July 21, 2026 (`635f3e4`) — **one page rearrangement**: the Run Order rail moves to the **left** as an Explorer-style task pane in **one line** through the grid skeleton — `[data-theme="luna"] #view-run .rv-body { grid-template-areas: "rail table vin" }`. (Luna's original second rearrangement — the `[data-arrange="desktop"] #view-home` captioned-desktop-icon grid — died with the Workflows cards; its `arrange:'desktop'` metadata stays on the registry but is now inert.)
...
| `.table-u` (+ `th`, `td`, `tr.row-danger`, `td.mono`) | sticky header bar (`background:var(--th-bg); color:var(--th-fg)`), 7px/12px `th` padding, 6px/12px `td` padding, hairline `td` bottom border, hover-row `background:var(--surface)` | `td.mono` uses the new `--font-mono` token; `tr.row-danger` tints the row with `--danger-weak` |
| `.file-pick` (+ `.file-pick-btn`, `.file-pick-name`) *(Sep 23, 2026)* | `<label>` wraps a visually-hidden `<input type=file>`, a `.btn-secondary` span, and a filename/count readout the view's change handler fills | native file-input chrome ("Choose Files \| No file chosen") can't be restyled directly; `:has(input:focus-visible)`/`:has(input:disabled)` forward the focus ring / disabled look onto the visible span since the real input is hidden. Used by Import and Data Sources; every programmatic reset of the input (incl. the `cloneNode`-and-replace pattern) must also reset the readout text |
...
**`--th-bg`/`--th-fg` (Sep 23, 2026 screenshot audit).** The `.table-u th` header bar was a fixed inversion (`background:var(--text); color:var(--bg)`) — correct in light themes but a near-white band across a dark UI in Dark, Midnight HC, and Gruvbox. Two tokens now drive it: base `:root` keeps `--th-bg: var(--text); --th-fg: var(--bg)` (light themes pixel-identical, since these resolve through the same `--text`/`--bg` the theme block itself overrides — no per-theme redeclaration needed for the light palettes). `[data-theme="dark"]` and `[data-theme="gruvbox-rail"]` override to `--surface-2`/`--text-2` (a tint, not an inversion); `[data-theme="midnight"]` overrides to `--surface-3`/`--text` (keeps full HC contrast). Encarta's `!important` header override and Stack Cleanup's surface header are untouched; Import's inventory-snapshot header, which re-declares the `.table-u th` recipe to beat the view's own `th` dialect, now reads the tokens too.
...
**Use the Add Dealer wizard** (App sidebar → **System Settings → Add Dealer**, `ViewAddDealer.html`; Code.gs **SECTION 35**), added **August 12, 2026**. It automates steps 1–5 and 7 of the manual runbook below; only the Pipedrive product map and the final activation still need a human. The manual runbook is kept as the fallback (and as the description of what the wizard writes).

### The wizard — three panels

**1. Basics.** Dealer name → auto-derived `dealer_key` (`ndKeyFromName_`, ALL_CAPS) and QR prefix (`ndPrefixFromName_`, case preserved); scraper location picked from the **live SCRAPERDATA locations** (or typed); LinkBuilder column (default `B`); notes. `getAddDealerBootstrap` supplies the existing keys, the live locations, the unconfigured-location diff, a preview of the next ORDERS column, and the dealer-folders container (the env-bound output folder) in one round trip. Inactive dealers appear as a **resume strip** so a half-built dealer is picked back up instead of re-created. Payload validation is server-side (`ndValidatePayload_` — key charset also excludes every character Sheets forbids in a tab name).

**2. Review & Create.** `createDealer(payload)` runs **every write as an idempotent ensure-step inside ONE script lock**, so it doubles as the repair action (the checklist's **"Create Missing Pieces"** sends only the key):

| Step | What it ensures |
|---|---|
| ORDERS column | Reuses the row's letter if set, else allocates **max(used)+1** (`ndNextOrdersCol_` — never reuses a gap; its own base-26 parser, because `normalizeOrderMatchCol_` caps at column 100 and a silently-ignored letter would REUSE a live column). Header row 1 = the dealer **name**; the column is `@`-formatted (QUERY mixed-type trap). |
| DEALERS row | Appended with `active=FALSE` (a half-built dealer is inert — `runDealer` hard-gates inactive dealers) and the baseline `filtering_rules` (`allowed_types: ["New","PO","CPO"]`, `exclude_status: ["OFFLOT"]`, `require_stock: true`). On an existing row it patches **only blank cells** — a manual edit always wins. |
| SF_VIN_LOGS tab | Tab named exactly `dealer_key`, headers `ORDER_ID \| VIN \| committed_at \| order_date`, frozen header row. An **orphan tab is reused with its history intact** (a leftover tab means re-activation — never wipe committed VINs). |
| Drive folders | Structure `<output folder>/<Dealer Name>/"QR Codes"` — anchored on the **env-bound `OUTPUT_FOLDER_ID`** (`ndDealersContainerFolder_`), never derived by walking another dealer's folder parents (the original derivation placed a new QR folder inside another dealer's folder — fixed Aug 12) and **never at Drive root** (My-Drive ownership = owner-only trash). A resolvable col-D id short-circuits everything; a resolvable col-E id is reused as the dealer folder (repair path); otherwise find-or-create the dealer folder by **name** in the container, then find-or-create **"QR Codes"** inside it, writing its id to col D. Unreachable output folder = a `failed` step. |

Each step returns `{status: created \| existing \| failed, detail}`; a per-artifact failure is recorded, not thrown, and is repaired by re-running.

**3. Setup checklist.** `getDealerChecklist(dealerKey)` is a **sheet-reads-only** completeness probe (no Pipedrive API call, so it works with no PD token configured) that mirrors the run pipeline's real gates. Blocking items: `dealers_row`, `orders_col` (the column actually exists on the ORDERS sheet), `vin_log`, `scraper_location` (**exact, case-sensitive** count of matching SCRAPERDATA rows — the same comparison `getDealerScraperData_` makes; downgraded to non-blocking when no inventory is imported yet), `filtering_rules` (parses to something real), `pipedrive_org` (a **PRIMARY** row with an org id), and `product_map` (`validateProductMapForRun_` over the dealer's allowed types — the exact run gate). `qr_folder` is **warn-only**, because `runNeedsQR_` skips the whole QR block for schemas with no QR field code. The Pipedrive items carry an `action` that jumps into **Dealer Rules** on the right tab. **"Activate Dealer"** (`activateDealer`) **re-runs the checklist server-side** before flipping DEALERS col L — the disabled client button is not a trust boundary.

**Hand-off to Dealer Rules.** `ViewRules.html` gained `rulesOpenDealer(key, name, tab)`: it injects the (still inactive) dealer into the active-only dropdown client-side, calls `CustomSelect.refresh()` to sync the enhanced facade, and opens the requested tab. The server side was already active-agnostic (`getDealerRulesData` / `getPipedriveDealerEditorData` resolve via `getDealerConfig_`, which scans every row).

### Unconfigured scraper locations (Import Data)

`importScraperData` now also returns **`unconfiguredLocations`** — a fail-soft diff (`ndDiffLocations_`, trim/lowercase compare) of the review's live locations against DEALERS col J across **ALL rows** (active *and* inactive, so a dealer mid-setup never re-flags). The Import review panel renders them as a warning banner (`#reviewUnconfigured`) with a per-location **"Add dealer →"** button that opens the wizard prefilled with the location and a suggested key. It can never fail an import.
...
3. In **Dealer Rules → Pipedrive**, map each post-normalization type (`New`, `PO`, `CPO`) to a product and set its **Schema** + **UTM** — this is the per-type output config (a matched type with no product/schema blocks the run). *(col O `type_rules` is dormant — not read by the run.)* **The wizard does not do this step.**
4. Set `filtering_rules` JSON — use `allowed_types`, `exclude_status: ["OFFLOT"]`, `require_stock: true` as a minimum baseline
5. Create tab in SF_VIN_LOGS named exactly by `dealer_key` with headers `ORDER_ID | VIN | committed_at | order_date`
6. Set `active` to TRUE *(the wizard's "Activate Dealer" button, gated on the checklist)*
