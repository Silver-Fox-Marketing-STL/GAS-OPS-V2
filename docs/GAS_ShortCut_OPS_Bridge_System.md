...
| `FEATURES` | 22 | **Per-row manual text** (July 2026 — no-QR dealers): typed into the Run Order table's Features inputs, written by `writeFeatures_` after matching (script-written value column like col J, not a template formula). Required (client + server fail-fast) for every VIN whose type resolves to a schema listing `FEATURES`; blank for all other rows |
...
| V | 22 | FEATURES | Script-written by `writeFeatures_` (July 2026) — per-row manual text typed in the Run Order table, VIN-aligned to the QUERY spill. **No formula; V2:V must stay empty in the template.** Blank unless the run supplied a `featuresMap` |

**Cols A–I are the QUERY spill zone.** Nothing should be written there in the template. Cols J (QR paths) and V (Features) are the script-written value columns. Cols K–U are ARRAYFORMULAs that auto-expand with QUERY output.
...
| `writeFeatures_` | `(outputDoc, matchedRows, featuresMap)` | Writes Features text into ORDERMATCH col V (`FIELD_TO_COL.FEATURES`), one batched write, VIN keyed off col E; `matchedRows[i]` ↔ sheet row `i+2` (contiguous QUERY spill). No-op without a map. |
