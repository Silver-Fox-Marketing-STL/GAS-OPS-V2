...
- `csv_schema` — references a row in CSV_SCHEMAS tab. **Now a FALLBACK** *(branch `feature/product-driven-schema`)* — see below.
- `utm` — `utm_medium` value appended to VDP URLs for QR generation. (Unchanged — UTM is still per type rule, applied per row.)
...
### Schema is derived from the Pipedrive product *(branch `feature/product-driven-schema` — not deployed)*

The CSV output **schema** (and therefore the grouping into CSV sheets) is **derived from the Pipedrive product each type maps to**, not set separately on the type rule. The one mapping the user already picks for billing — the product in the `PIPEDRIVE` tab's `product_map` — now drives billing, the CSV layout, **and** the grouping. This resolves "schema ≠ template": the *product* carries the template identity. `type_rules.csv_schema` is the **fallback** used only when the product has no `schema` set. *(This supersedes the earlier `output`-label approach — branch `feature/csv-output-grouping`, which is not on this branch.)*

- **Resolution** (`resolveRuleSchema_(rule, productMap)`): if the rule matches a **specific** type and `product_map[type].schema` is set, use that schema; otherwise use `rule.csv_schema`. A `*` catch-all rule has no single type, so it always uses the fallback.
- **Grouping** (`csvOutputGroups_`): type rules whose types resolve to the **same** schema merge into one CSV sheet. The run pipeline reads the dealer's product maps via `getCsvProductMaps_(dealerKey, sourceSplit)` (safe-empty `{}` when Pipedrive is unconfigured → every rule falls back to `csv_schema`). For a `source_split` dealer the main and secondary outputs are grouped by their **own** product maps (`product_map` vs `source_product_map[group]`), so the subprime site can use a different layout. See **Pipedrive Integration → PIPEDRIVE config tab** for the per-mapping `schema` key.

### Multi-rule dealer configurations

**Auffenberg Hyundai:** `[{"match":"PO","csv_schema":"SCP","utm":"VDP_ShortCut"}]`

**Bommarito Cadillac:** `[{"match":"New","csv_schema":"SCP","utm":"VDP_ShortCut_New"},{"match":"PO","csv_schema":"SCP","utm":"VDP_ShortCut_Used"},{"match":"CPO","csv_schema":"SCP","utm":"VDP_ShortCut_Used"}]`
...
**CSV output naming:** sheets are named by the **resolved schema**, not by type. One resolved schema → a single `CSV` sheet; multiple → one sheet per schema named `CSV_<SCHEMA>` (e.g. `CSV_SC`, `CSV_SCP`). *(This changed with `feature/product-driven-schema` — previously one sheet per type rule, `CSV_NEW`/`CSV_PO`/`CSV_CPO`. On deploy this regroups 4 multi-rule dealers' outputs: Bommarito Cadillac collapses to a single `CSV`; CDJR of Columbia and MBCC go 3→2 sheets; Serra Honda's two sheets are renamed to `CSV_SC`/`CSV_SCP`. Single-`*`-rule dealers are unchanged.)*
...
| E | `product_map` | JSON `{type: {product_id, variation_id?, schema?}}` — per-type Pipedrive product (`New`/`PO`/`CPO`/`CPO-EL`), optionally a specific variation and an optional **`schema`** (CSV-schema key — sets that product's CSV layout/grouping; blank → the type rule's `csv_schema` fallback — see *Type Rules System*). A bare `{type: product_id}` is still tolerated. *(`schema` added on `feature/product-driven-schema`; `buildLineItems_` ignores it.)* |
...
| L | `source_product_map` | JSON `{ "<sourceGroupName>": { type: {product_id, variation_id?, schema?} } }` — per-type products (each with an optional `schema` like col E) for a `source_split` dealer's **secondary** CSV output (e.g. `{AUTOLOANPRO: {New:{…}, PO:{…}}}`), pushed onto the **same** deal as the main map (col E). `{}` (empty) for non-source-split dealers. *(`PDCFG.SOURCE_PRODUCT_MAP = 11`. See *Per-source product config* below.)* |

A dealer has a **PRIMARY row plus one row per `billing_split` group**, each with its own org **and** its own per-type product map. This is how **Mercedes-Benz of Creve Coeur's two Pipedrive orgs** (MB Creve Coeur for cars + Sprinter Creve Coeur for vans) are handled — the split run's two billing accounts map to two PIPEDRIVE rows, each pushing to its own org. **Type Rules (col O) are unchanged** by this feature. Server: `getPipedriveDealerConfig_(dealerKey, group)`, `getPipedriveDealerEditorData(dealerKey)` (now also returns `globalRules`, so the panel can list the rules that are overridable), `savePipedriveDealerConfig(dealerKey, rows)` (replaces all of a dealer's rows); the tab is auto-created by `getOrCreatePipedriveSheet_`.

**Per-mapping `schema` drives the CSV output** *(branch `feature/product-driven-schema`)*: each `product_map` / `source_product_map` entry's optional `schema` sets the CSV layout (and grouping) for that product — so the product the user picks for billing also picks the template. The run pipeline reads these maps via `getCsvProductMaps_` and groups CSV sheets by the **resolved** schema (`resolveRuleSchema_` → `product_map[type].schema` else `type_rules.csv_schema`); safe-empty `{}` when Pipedrive is unconfigured, so it falls back to the type rule. The Dealer Rules → Pipedrive product grids (main + source-split) carry a **Schema** column for this. See **Type Rules System → Schema is derived from the Pipedrive product**.
...
- **Config.** The secondary products live in `PIPEDRIVE` tab **col L `source_product_map`** (`PDCFG.SOURCE_PRODUCT_MAP = 11`): JSON `{ "<sourceGroupName>": { type: {product_id, variation_id?} } }`. The main-site products stay in `product_map` (col E). Parsed as `sourceProductMap` by `pdRowToConfig_` and persisted by `savePipedriveDealerConfig`.
- **Editor (`ViewRules.html`).** `getPipedriveDealerEditorData` now also returns `sourceSplit` (from `getSourceSplit_`). Whenever the dealer has a `source_split`, each deal block renders a **secondary per-type product grid** ("Products for the `<groupName>` output") below the main grid — driven generically off `filtering_rules`, the same way `billing_split` drives the per-deal blocks (no per-dealer code).
...
### Install cost & Design no-charge variation *(branch `feature/pipedrive-install-cost`, off `pipedrive-integration`)*
...
### Billing PDF attachment *(branch `feature/pipedrive-billing-pdf`, off `pipedrive-integration`)*
...
### Finalize-from-card — method-first finalization *(branch `feature/pipedrive-finalize-flow`, off `pipedrive-integration`)*
