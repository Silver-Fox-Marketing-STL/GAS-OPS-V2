...
|---|---|---|
| June 19, 2026 | — | **Pipedrive API integration — branch `pipedrive-integration`, pending deploy + live config (NOT shipped; no `clasp push`).** Pushes a **finalized** run to Pipedrive as a deal with per-type product line items, as a **separate, explicit step after** in-system finalization (never automatic; never before the RUN_LOG row + col-D Deal ID exist). Code.gs **Section 31**. (1) **Secrets in ScriptProperties** (`PD_API_TOKEN`, `PD_COMPANY_DOMAIN`, `PD_DEFAULT_PIPELINE_ID`/`_STAGE_ID`/`_CURRENCY`) — never in repo/sheet; `setupPipedriveSecrets` validates via a live `GET /users/me` before saving; `getPipedriveStatus` never returns the token. (2) **New `PIPEDRIVE` tab in SF_DEALER_CONFIG**, one row per `(dealer_key, group)`, cols A–K (`dealer_key, group, org_id, org_name, product_map` {type:product_id}, `deal_title_template, pipeline_id, stage_id, currency, field_map, active`) — a PRIMARY row + one row per `billing_split` group, each with its own org **and** product map (how MB Creve Coeur's two orgs are handled). Type Rules (col O) unchanged. (3) **`pushRunToPipedrive(dealerKey, runRowIndex, mode, existingDealId)`** (`mode` `create`/`link`); line items via `buildLineItems_` (qty = gross − dupes, shared products summed, price by currency). **Idempotent / retry-safe:** Deal ID written to **RUN_LOG col D** the instant PD returns it (numeric col D ⇒ already-created, dup guard); a `pd_push_<row>` ScriptProperties record resumes a retry — no duplicate deal, no double-attach; on failure → error + retry, no rollback. (4) **`pdFetch_` never throws** (`{ok,status,data,error}`; v1 default, v2 for org custom-field reads; one bounded 429 backoff). **Org→deal `field_map`** (resolved by `pdResolveFieldMap_`) ships dark (empty = no-op). Config UI populated LIVE from PD (`getPipedriveConfigBootstrap` cached ~10 min, `searchPipedriveOrganizations`). UI: Rules Editor **Pipedrive panel**, ViewRun **Push to Pipedrive** sub-block on finalized cards, ViewVinLog per-run push/retry. See the **Pipedrive Integration** section below. |
...
A new **`PIPEDRIVE` tab**, **one row per `(dealer_key, group)`**. Cols A–K:
...
| E | `product_map` | JSON `{type: product_id}` — per-type Pipedrive product (`New`/`PO`/`CPO`/`CPO-EL`). |
...
| J | `field_map` | JSON array of org→deal custom-field mappings (see below). Empty = no-op. |
| K | `active` | TRUE = this group's push is enabled. |

A dealer has a **PRIMARY row plus one row per `billing_split` group**, each with its own org **and** its own per-type product map. This is how **Mercedes-Benz of Creve Coeur's two Pipedrive orgs** (MB Creve Coeur for cars + Sprinter Creve Coeur for vans) are handled — the split run's two billing accounts map to two PIPEDRIVE rows, each pushing to its own org. **Type Rules (col O) are unchanged** by this feature. Server: `getPipedriveDealerConfig_(dealerKey, group)`, `getPipedriveDealerEditorData(dealerKey)`, `savePipedriveDealerConfig(dealerKey, rows)` (replaces all of a dealer's rows); the tab is auto-created by `getOrCreatePipedriveSheet_`.

### Line items
`buildLineItems_(billing, productMap, products, currency)` turns the run's billing totals into deal products. **Quantity per type = net of dupes** (`totalNew − newDupes`, `totalPO − poDupes`, etc., read from `readBillingTotals_`); types that map to the **same** product are **summed** into one line item; `item_price` is resolved from the product's `prices[]` by the target currency (falling back to the first listed price).
...
- **`create`** makes a new deal on the group's org (title from the template, pipeline/stage/currency from the row or the defaults), attaches the per-type products, then sets mapped deal fields.
...
### Org→deal field mapping (ships dark)
`field_map` (resolved by `pdResolveFieldMap_`) reads the organization's custom-field values (v2) and sets the corresponding **deal** custom fields. An **empty `field_map` is a no-op**, so the push works without it until field mappings are configured. v1 deal custom fields are written as **top-level hash keys** (with a `<key>_currency` companion for monetary fields) — *not* nested under a `custom_fields` object (that wrapper is v2-only).

### Config UI populated live from Pipedrive
So Pipedrive stays the single source of truth, the config editor reads its catalog live: `getPipedriveConfigBootstrap(refresh)` returns products + deal/org field definitions (cached ~10 min via `CacheService`), and `searchPipedriveOrganizations(term)` powers the org picker.

### UI surfaces
- **Rules Editor (`ViewRules.html`)** — a new **Pipedrive panel**: connection setup, a per-group org picker, a type→product grid, and a field-map builder driven by the live `dealFields` / `orgFields`.
