...
### Bridge System Documentation | Last Updated: June 24, 2026
...
**As of:** June 2026
**Development branch:** `main` — `feature/health-monitoring` merged June 2026; **`pipedrive-integration` merged to `main` June 24, 2026 (v2.12 — deployed)**. `main` is the single deployed branch.
...
## Vehicle-Type Registry *(`feature/dynamic-vehicle-types` — branch-only; NOT merged to `main`, NOT deployed)*

> **Status:** committed on `feature/dynamic-vehicle-types` (4 commits) but **not merged to `main` and not `clasp push`ed**, and **inert on the live system** — the `PIPEDRIVE_SETTINGS` `vehicle_types` row doesn't exist yet, so the registry returns exactly the canonical four and behavior is byte-identical to before. Documented here ahead of deploy.
