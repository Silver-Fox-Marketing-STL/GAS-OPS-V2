...
Deal-field mapping is **global** (not per-dealer). A simple **`PIPEDRIVE_SETTINGS`** key/value tab (`key | value`, auto-created by `getOrCreatePipedriveSettingsSheet_`) holds one row, key **`deal_field_rules`**, whose value is a JSON **array of rules**. Each rule sets **one** deal field, in one of two modes:
...
A **`PIPEDRIVE` tab**, **one row per `(dealer_key, group)`** (per-dealer org, product map, deal defaults, and overrides of the global rules). Cols A–K:
...
| E | `product_map` | JSON `{type: {product_id, variation_id?}}` — per-type Pipedrive product (`New`/`PO`/`CPO`/`CPO-EL`), optionally a specific variation. A bare `{type: product_id}` is still tolerated. |
...
`pdFetch_(method, path, payload, opts)` **never throws** — it returns `{ok, status, data, error}`. The token is passed as `?api_token=`; the **v1** base is used by default and **v2** for organization custom-field reads; it does **one bounded 429 backoff**; it parses Pipedrive's `{success, data, additional_data}` envelope. This is the same isolation discipline as the non-fatal stats/dashboard writes — a Pipedrive API failure can never fail a run.

### Org→deal field resolution (global rules + per-dealer overrides; ships dark)
`pdResolveDealFields_(orgId, globalRules, overrides, currency)` (which **replaced `pdResolveFieldMap_`**) is the runtime resolver. It (1) builds the **effective** rule list — each global rule is dropped if its override is `{off:true}`, replaced wholesale if the override is a replacement rule, or kept as-is otherwise; (2) reads only the org keys the effective rules reference (`pdCollectOrgKeys_` walks copy `org_field`s + conditional condition fields); (3) evaluates each effective rule against the org — *copy* rules read-and-translate, *conditional* rules pick `then_value`/`else_value` via `pdOrgGroupMatches_` — writing each via `pdSetDealField_`. The result is a **flat top-level** `{dealFieldKey: value}` map (v1 deals API takes custom fields as top-level 40-char hash keys, with a `<key>_currency` companion for monetary fields — *not* nested under a `custom_fields` object, which is v2-only). **No global rules (or every rule overridden off) = a no-op**, so the push works before any field mapping exists.
...
### UI surfaces
- **System Settings → Pipedrive Settings (`ViewPipedriveSettings.html`, new)** — the **global** screen: the **connection setup** (token + domain + defaults, validated live; moved here out of the Dealer Rules panel) and the **global deal-field rules builder** (copy / conditional rules against the live `dealFields` / `orgFields`).
- **Dealer Rules → Pipedrive panel (`ViewRules.html`)** — now **per-dealer only**: connection state is read-only (it points to System Settings when not connected); a per-group org picker; a type→product grid with a **variation selector** (lazy-loaded per product); and a **Deal-Field Overrides** section listing each global rule with **Use-global / Off / Override…**, where the inline "Override…" editor **reuses a shared, context-parameterized rule-card editor** (`psRenderRuleCard_` / `psSerializeOneRule_` / `psDeserializeOneRule_` / `psRegisterRuleCtx_`) shared with the global settings screen so there is one implementation.
...
- **Run Order (`ViewRun.html`)** — each **finalized** card gains a **Push to Pipedrive** sub-block (create / link / retry); it appears only after the card is finalized (a finalized card has the `rowIndex` the push targets).
- **VIN Logs (`ViewVinLog.html`)** — a per-run **Push to Pipedrive** / retry action for finalized runs.
