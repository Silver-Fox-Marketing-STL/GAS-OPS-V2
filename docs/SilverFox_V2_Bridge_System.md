...
- `deal_label` — label for the second deal ID field in the Run Dealer modal (defaults to `<group_name> Deal ID`).
...
**Effects when active and the run contains group vehicles:** the modal requires a second Pipedrive deal ID; the output doc gets **BILLING** (primary account — group units excluded; carries the not-found list) plus **BILLING_<group_name>** (created on demand; identical five-section layout); **two RUN_LOG rows** are written (one per account, each with its own deal ID/totals/produced VINs; notes col U = `SPLIT:PRIMARY` / `SPLIT:<group>`; same output_doc_id) plus two ORDER_STATS rows. Commit/rollback in the VIN Log Updater works per account against the dealer's **single** VIN log tab — group VINs are logged under the group's deal ID; duplicate detection is identifier-based and unaffected. Zero group units in a run → BILLING_<group> is still written (zeroed) but the second RUN_LOG row is skipped (note: `split: 0 <group> units`). Sheet-side QUERYs that count RUN_LOG rows see a split run as two rows — filter on `notes` if exact run counts matter.
...
| `pasteVinsAndRun` | `(dealerKey, vins, dealId, runId, bypassFilters, userKey, splitDealId)` | Resolves QR base path for `userKey` from USER_PROFILES, persists selection, writes VINs to ORDERS, calls `runDealer` (passing preloaded config to avoid a redundant SF_DEALER_CONFIG read). Throws if the dealer has a `billing_split` and `splitDealId` is blank. Returns result object. |
| `runDealer` | `(dealerKey, dealId, runId, bypassFilters, qrBasePath, preloadedConfig, splitDealId)` | Main entry point. `preloadedConfig` is optional — if provided (from `pasteVinsAndRun`), skips the `getDealerConfig_` call. `splitDealId` is the second deal ID for billing-split dealers. Returns `{outputFolderUrl, runLogRowIndex, splitRunLogRowIndex, splitGroupName, dealerName, producedVinCount}`. |
...
- **MBCC/Sprinter shared inventory — resolved in code, pending deploy:** Option B chosen (one run, two billing outputs). Implemented on branch `feature/billing-split` via the `billing_split` key in `filtering_rules` (see Billing split section). Remaining steps: merge + `clasp push`, then add the `billing_split` JSON to MBCC's col W (`field: "model", op: "contains", values: ["Sprinter", "Metris"]`) and live-test per the v2.9 changelog entry.
