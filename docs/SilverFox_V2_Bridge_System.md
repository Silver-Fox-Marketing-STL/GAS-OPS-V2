...
- **Orchestrator — `attachBillingPdfToDeal_(dealId, outputDocId, group, meta)`** = skip if already attached, else generate + attach. **`pdDebugBillingPdf(dealId, outputDocId, group)`** is a TEMPORARY diagnostic (run via a no-arg wrapper) confirming the export + upload live — to be removed after verification.
