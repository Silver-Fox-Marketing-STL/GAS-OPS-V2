...
- **`getDashboardView()`** — returns the DASHBOARD tab as `{rows}` (display strings, no Date objects). Home renders it below the workflow cards and reloads it on every Home visit (`VIEW_SHOWN` + a Refresh button), so run numbers refresh without a re-import.
